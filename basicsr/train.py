...
def train_pipeline(root_path):
    ...
    if resume_state:
        ...
        if 'best_metric' in resume_state:
            logger.info(f"Resuming from epoch: {resume_state['epoch']}, iter: {resume_state['iter']}, best_metric: {resume_state['best_metric']:.4f}.")
    ...
    # Mixed precision için GradScaler oluşturuluyor.
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while train_data is not None:
            ...
            # Mixed precision optimize_parameters çağrısı:
            model.optimize_parameters(current_iter, scaler)
            ...
            if opt.get('val') is not None and (current_iter % opt['val']['val_freq'] == 0):
                if len(val_loaders) > 1:
                    logger.warning('Multiple val sets are only tested in a for-loop manner.')
            ...
            if current_iter % opt['logger']['save_checkpoint_freq'] == 0:
                logger.info('Saving latest models and states.')
